...
int main(int argc, char** argv){
    ...
    DS<float> *T, *T_full, *T_N, *M, *K, *b, *L;
    DS<int> *dirichlet_indices, *neumann_indices;

    DS<DS<float>*> *Result,*M_locals,*K_locals,*b_locals,*L_locals;
    ...
    while( t <= tf ){
        ...
        SDDS<DS<float>*>::create(&L_locals, nelems, ARRAY);
        ...
        for(int e = 0; e < nelems; e++){
            ...
            SDDS<DS<float>*>::insert(K_locals, e, FEM::calculate_local_K(current_elem));
            ...
            SDDS<DS<float>*>::insert(L_locals, e, FEM::calculate_local_L(current_elem));
            cout << "OK\n\n";
        }
        ...
        SDDS<float>::create(&L, nnodes, nnodes, MATRIX); Math::zeroes(L);
        ...
        for(int e = 0; e < nelems; e++){
            ...
            //Se extrae la matriz L del elemento actual y se envía a ensamblaje
            SDDS<DS<float>*>::extract(L_locals,e,&temp);
            FEM::assembly(L, temp, current_elem, true);
        ...
        FEM::apply_Dirichlet(nnodes, free_nodes, &b, K,L, Td, dirichlet_indices);
        FEM::apply_Dirichlet(nnodes, free_nodes, &K, dirichlet_indices);
        FEM::apply_Dirichlet(nnodes, free_nodes, &L, dirichlet_indices);
        ...
        /*
            Se procede a ejecutar la ecuación de transferencia de calor en su versión discretizada con Forward Euler:
                        ...
                        T^(i+1) = T^i + M^(-1) * delta_t * ( b - L * T^i + K * T^i )
        ...
        */
        ...
        DS<float>* temp2 = Math::product(L,T);
         Math::sum_in_place(temp, temp2);
        ...
        DS<float>* temp3 = Math::inverse_Cholesky(M);
        //SDDS<float>::show(temp2,false);
        DS<float>* temp4 = Math::product( temp3, b );
        Math::sum_in_place(T, temp4 );
