    ...
    private:
        ...
        */
        static float calculate_local_J(Point* P1, Point* P2, Point* P3){
            return  std::abs((P2->get_x() - P1->get_x())*(P3->get_y() - P1->get_y()) - (P3->get_x() - P1->get_x())*(P2->get_y() - P1->get_y()));  //o_O
        }
        ...
        */
        static float calculate_local_Area(Point* P1, Point* P2, Point* P3){
            return fabs((P1->get_x()*( P2->get_y() - P3->get_y() ) + P2->get_x()*( P3->get_y() - P1->get_y() ) + P3->get_x()*( P1->get_y() - P2->get_y() ))/2);
        }
        ...
        */
        ...
        /*
            Función para calcular la matriz N para un elemento de la malla.

            La matriz N surge durante la creación de las fórmulas para la matriz L
            del proceso MEF2D, y su correspondiente fórmula quedó definida de la
            siguiente manera:
                                ...
                                N = [ 1/2   1/2 ]
                                    [ 1      1  ]
        */
        static void calculate_N(DS<float>* N){
            //Se definen los elementos de la matriz de acuerdo a la fórmula posición por posición
            SDDS<float>::insert(N,0,0,1/2); SDDS<float>::insert(N,0,1,1/2);
            SDDS<float>::insert(N,1,0,1/2); SDDS<float>::insert(N,1,1,1/2);
            SDDS<float>::insert(N,2,0,1); SDDS<float>::insert(N,2,1,1);
        }
    ...
    public:
        ...
        */
        static DS<float>* calculate_local_M(Element* e){
            ...
            //Se definen los elementos de la matriz de acuerdo a la fórmula posición por posición
            SDDS<float>::insert(M,0,0,1/10); SDDS<float>::insert(M,0,1,1/5); SDDS<float>::insert(M,0,2,1/20);
            SDDS<float>::insert(M,1,0,1/5); SDDS<float>::insert(M,1,1,1); SDDS<float>::insert(M,1,2,1/5);
            SDDS<float>::insert(M,2,0,1/20); SDDS<float>::insert(M,2,1,1/5); SDDS<float>::insert(M,2,2,1/10);

            //Se multiplica el contenido de la matriz M por el factor rho*Cp*J/24
            Math::product_in_place(M, J/42);
        ...
        */
        static DS<float>* calculate_local_K(Element* e){
            ...
            //Se multiplica el contenido de la matriz K por el factor J/(D^2)*6
            Math::product_in_place(K, J/((D*D)*6));
        ...
        static DS<float>* calculate_local_L(Element* e){
            //Se preparan las variables para el proceso
            DS<float> *A, *B, *N, *L;
            ...
            //Se calcula la matriz B
            calculate_B(B);
            //Se definen la matrix N con dimensiones 3 x 2
            //Esto corresponde a las dimensiones resultantes para la aplicación del MEF
            //a un problema 2D
            SDDS<float>::create(&N,3,2,MATRIX);
            //se calcula la matriz N
            calculate_N(N);
            //Se efectúa N * A * B y el resultado se almacena en <L>
            DS<float>* temp = Math::product( N, A );
            L = Math::product( temp, B );
            

            //Se multiplica el contenido de la matriz K por el factor J/D*12
            Math::product_in_place(L, J/(D*12));

            //Las matrices A,B,N  ya no son necesarias, por lo
            //que se liberan sus espacios en memoria asignados
            SDDS<float>::destroy(A); 
            SDDS<float>::destroy(B);
            SDDS<float>::destroy(N); 
        ...
        */
        static DS<float>* calculate_local_b(Element* e){
            ...
            SDDS<float>::insert(b,1,0,3/2);
            SDDS<float>::insert(b,2,0,11/6);

            //Se multiplica el contenido de la matriz b por el factor Q*J/6
            Math::product_in_place(b, J/20);
        ...
        */
        static void apply_Dirichlet(int nnodes, int free_nodes, DS<float>** b, DS<float>* K, DS<float>* L,  float Td, DS<int>* dirichlet_indices){
                ...
                if(!bres){
                    ...
                    for(int j = 0; j < nnodes; j++){
                        ...
                        if(bres){
                            ...
                            SDDS<float>::extract(L,i,j,&temp2);
                            acum2 += Td*temp;
                        }
                    }
